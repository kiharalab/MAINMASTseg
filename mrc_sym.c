...
bool readmrc(MRC *mrc, char *filename){
 ...
 fseek(fpin,4*(256-54)+nsymbt,SEEK_CUR);
	...
	if(mrc->widthx!= mrc->widthy || 
	 mrc->widthx !=  mrc->widthz ||
	 mrc->widthy !=  mrc->widthz){
...
void ShowModelChain(MRC *m,POINTS *p,GRAPH *g){
 int i,j,k;
 int Natm=1;
 double tmp[3];

 char code[35]="ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789";
 char ch;
 ...
 for(i=0;i<p->Ncd;i++){
  ...
  tmp[1]=p->cd[i][1]*m->widthx+m->orgxyz[1];
  tmp[2]=p->cd[i][2]*m->widthx+m->orgxyz[2];
  //printf("ATOM  %5d  CA  ALA %c%4d    ",Natm,ch,Natm);
  printf("ATOM  %5d  CA  ALA %c%4d    ",i+1,ch,i+1);
  printf("%8.3f%8.3f%8.3f%6.2f%6.2f\n",tmp[0],tmp[1],tmp[2],1.0,p->dens[i]);
  Natm++;
 }
...
void ShowModelChainCIF(MRC *m,POINTS *p,GRAPH *g){
 ...
 char code[61]="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnooqrstuvwxyz123456789";
 char ch;
...
bool InitTree(TREE *t,bool mode) {
 ...
 if(mode){
  ...
  for(int i=0;i<t->Nnode;i++)
   if((t->node[i]=(NODE *)malloc(sizeof(NODE)))==NULL)
    return true;

  printf("#end node\n");

  //if((t->mv=(MOVE **)malloc(sizeof(MOVE *)*t->Etotal*t->Etotal))==NULL)
  // return true;
  //for(int i=0;i<t->Etotal*t->Etotal;i++)
  // if((t->mv[i]=(MOVE *)malloc(sizeof(MOVE)))==NULL)
  //  return true;
...
bool CopyTree(TREE *in, TREE *out,bool mode){
 ...
 if(mode){
  ...
  for(int i=0;i<out->Nnode;i++)
   if((out->node[i]=(NODE *)malloc(sizeof(NODE)))==NULL)
    return true;
  ...
  if((out->mv=(MOVE **)malloc(sizeof(MOVE *)*out->Etotal*out->Etotal))==NULL)
   return true;
  for(int i=0;i<out->Etotal*out->Etotal;i++)
   if((out->mv[i]=(MOVE *)malloc(sizeof(MOVE)))==NULL)
    return true;
