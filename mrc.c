 ...
 */
 ...
 fclose(fpin);
 mrc->orgxyz[0]= mrc->ncstart*mrc->widthx + mrc->orgxyz[0];
 mrc->orgxyz[1]= mrc->nrstart*mrc->widthy + mrc->orgxyz[1];
 mrc->orgxyz[2]= mrc->nsstart*mrc->widthz + mrc->orgxyz[2];
...
bool meanshift(MRC *m,POINTS *p){
 ...
 //malloc
 if((p->cd=(double **)malloc(sizeof(double *)*m->NumVoxels*12))==NULL)
  return true;
 if((p->origrid=(int **)malloc(sizeof(int *)*m->NumVoxels))==NULL)
  return true;
...
void ShowModelChainCIF(MRC *m,POINTS *p,GRAPH *g){
 int i,j,k;
 int Natm=1;
 double tmp[3];
...
printf("_atom_site.pdbx_PDB_ins_code\n"); 
...
printf("_atom_site.pdbx_formal_charge\n"); 
printf("_atom_site.auth_seq_id\n"); 
printf("_atom_site.auth_comp_id\n"); 
printf("_atom_site.auth_asym_id\n"); 
printf("_atom_site.auth_atom_id\n"); 
printf("_atom_site.pdbx_PDB_model_num\n");

 printf("#org: %f %f %f\n",m->orgxyz[0],m->orgxyz[1],m->orgxyz[2]);

 for(int c=0;c<g->Nchain;c++){
  ch=code[c];
 
 for(i=0;i<p->Ncd;i++){
  if(g->cid[i]!=g->chain[c])
   continue;
  tmp[0]=p->cd[i][0]*m->widthx+m->orgxyz[0];
  tmp[1]=p->cd[i][1]*m->widthx+m->orgxyz[1];
  tmp[2]=p->cd[i][2]*m->widthx+m->orgxyz[2];
  printf("ATOM   %-6d C CA  . ALA %c  1 %5d ? ",i+1,ch,i+1);
  printf("%.3f %.3f %.3f %.2f %.2f",tmp[0],tmp[1],tmp[2],1.0,p->dens[i]);
  printf(" ? %d  ALA %c CA  1\n",i+1,ch);
 }
...
void ShowModelChainCIF_Movie(MRC *m,POINTS *p,GRAPH *g){
 int i,j,k;
 int Natm=1;
 double tmp[3];
 int cid;
...
printf("_atom_site.pdbx_PDB_model_num\n");
 ...
 int mcnt=0;
 for(int mod=0;mod<Nt;mod++){

	 int v1=tree[mod]->id1;
	 int v2=tree[mod]->id2;
	 int tmp_cid=g->cid[v2];
	 //update cid in the tree
  	for(int j=0;j<=mod;j++){
  	 if(g->cid[tree[j]->id1]==tmp_cid)
  	  g->cid[tree[j]->id1]=g->cid[v1];
 ...
 for(i=0;i<p->Ncd;i++){
  ...
  tmp[2]=p->cd[i][2]*m->widthx+m->orgxyz[2];
  //printf("ATOM   %-6d C CA  . ALA %c  1 %5d ? ",i+1,ch,i+1);
  printf("ATOM   %-6d C CA  . ALA %d  1 %5d ? ",i+1,cid,i+1);
  printf("%.3f %.3f %.3f %.2f %.2f",tmp[0],tmp[1],tmp[2],1.0,p->dens[i]);
  //printf(" ? %d  ALA %c CA  1\n",i+1,ch);
  printf(" ? %d  ALA %d CA  %d\n",i+1,cid,mcnt);
 }
...
bool GenerateSegmentMrc(MRC *m,POINTS *p,GRAPH *g){
 int i,j,k;
 int Natm=1;
 double tmp[3];
...
*/
 ...
 //copy
	...
	out.xlen=m->xlen; out.ylen=m->ylen; out.zlen=m->zlen;

	out.alpha=m->alpha;
	out.beta=m->beta;
	out.gamma=m->gamma;
	
	out.mapc,out.mapr,out.maps;
	out.dmin,out.dmax,out.dmean;
	out.ispg=m->ispg;
	out.nsymbt=m->nsymbt;
	out.orgxyz[0]=m->orgxyz[0];
	out.orgxyz[1]=m->orgxyz[1];
	out.orgxyz[2]=m->orgxyz[2];
	out.NumVoxels=m->NumVoxels;
	out.widthx=m->widthx; out.widthy=m->widthy; out.widthz=m->widthz;
	out.Nact=m->Nact;
	out.ncstart=m->ncstart;
	out.nrstart=m->ncstart;
	out.nsstart=m->ncstart;
 ...
 for(int c=0;c<g->Nchain;c++){
  ...
  //init
  	for(int ii=0;ii<out.NumVoxels;ii++)
		out.dens[ii]=0.00;
...
*/
 	 ...
 	 //tmp[2]=p->cd[i][2]*m->widthx+m->orgxyz[2];
		 ...
		 if(m1 != i)
		  continue;
		 idx=p->origrid[ii][2]*xydim+p->origrid[ii][1]*xdim+p->origrid[ii][0];
		 out.dens[idx]=m->dens[idx];
		 //printf("ch%d idx= %d dens= %f\n",c,idx,out.dens[idx]);
		 //printf("ch%d idx= %d/%d dens= %f\n",c,idx,out.NumVoxels,0.0);
		 /*
		  tmp[0]=p->origrid[ii][0]*m->widthx+m->orgxyz[0];
		  tmp[1]=p->origrid[ii][1]*m->widthx+m->orgxyz[1];
		  tmp[2]=p->origrid[ii][2]*m->widthx+m->orgxyz[2];
		  printf("ATOM   %-6d C CA  . ALA %c  1 %5d ? ",ii+1,ch,ii+1);
		  printf("%.3f %.3f %.3f %.2f %.2f",tmp[0],tmp[1],tmp[2],1.0,p->dens[i]);
		  printf(" ? %d  ALA %c CA  1\n",ii+1,ch);
		  */
	 	}
 	}
	sprintf(filename,"region%d.mrc",c);
	//Write mrc format
	write_mrc(&out,filename);
	//break;
 }
}

bool write_mrc(MRC *mrc,char *fname){
 FILE *fpin;//actually it is fpout
 int mode=2;
 int res;
 printf("#Generating...%s\n",fname);

 if((fpin=fopen(fname,"wb")) == NULL){ 
  fprintf(stderr,"Can't open %s\n",fname); 
  return(true); 
 }

 fwrite(&(mrc->xdim),sizeof(int),1,fpin);
 fwrite(&(mrc->ydim),sizeof(int),1,fpin);
 fwrite(&(mrc->zdim),sizeof(int),1,fpin);

 fwrite(&(mode),sizeof(int),1,fpin);

 fwrite(&(mrc->ncstart),sizeof(int),1,fpin);
 fwrite(&(mrc->nrstart),sizeof(int),1,fpin);
 fwrite(&(mrc->nsstart),sizeof(int),1,fpin);

 //mxyz
 fwrite(&(mrc->mx),sizeof(int),1,fpin);
 fwrite(&(mrc->my),sizeof(int),1,fpin);
 fwrite(&(mrc->mz),sizeof(int),1,fpin);
 printf("#MXYZ: %d %d %d\n",mrc->mx,mrc->my,mrc->mz);

 //xyz len
 res=fwrite(&(mrc->xlen),sizeof(float),1,fpin);
 res=fwrite(&(mrc->ylen),sizeof(float),1,fpin);
 res=fwrite(&(mrc->zlen),sizeof(float),1,fpin);
 printf("#LenXYZ: %f %f %f\n",mrc->xlen,mrc->ylen,mrc->zlen);


 //abg
 res=fwrite(&(mrc->alpha),sizeof(float),1,fpin);
 res=fwrite(&(mrc->beta),sizeof(float),1,fpin);
 res=fwrite(&(mrc->gamma),sizeof(float),1,fpin);
 printf("#abg: %f %f %f\n",mrc->alpha,mrc->beta,mrc->gamma);
 ...
 mrc->mapr=2;
 mrc->maps=3;
 res=fwrite(&(mrc->mapc),sizeof(int),1,fpin);
 res=fwrite(&(mrc->mapr),sizeof(int),1,fpin);
 res=fwrite(&(mrc->maps),sizeof(int),1,fpin);
 ...
 mrc->dmean=0.50;
 mrc->ispg=0;
 res=fwrite(&(mrc->dmin),sizeof(float),1,fpin);
 res=fwrite(&(mrc->dmax),sizeof(float),1,fpin);
 res=fwrite(&(mrc->dmean),sizeof(float),1,fpin);
 res=fwrite(&(mrc->ispg),sizeof(int),1,fpin);
 printf("#dmax,dmin, dmean, ispg: %f %f %f %d\n",mrc->dmax,mrc->dmin,mrc->dmean,mrc->ispg);

 //93-96
 mrc->nsymbt=0;
 res=fwrite(&(mrc->nsymbt),sizeof(int),1,fpin);
 //97-196 Extra
 fseek(fpin,4*25,SEEK_CUR);

 //197-208 ORIGIN
 res=fwrite(&(mrc->orgxyz),sizeof(float),3,fpin);
 printf("#orgXYZ: %f %f %f\n",mrc->orgxyz[0],mrc->orgxyz[1],mrc->orgxyz[2]);
 
 //ignore MAP 209-212
 char text[4]="MAP";
 res=fwrite(text,sizeof(char)*4,1,fpin);


 char machst[4];
 machst[0] = 0x44;

 res=fwrite(machst,sizeof(char)*4,1,fpin);

 fseek(fpin,4*(256-54)+mrc->nsymbt,SEEK_CUR);

 int ordermode=1;
 for(int i = 0; i<mrc->NumVoxels; ++i)
	   res=fwrite(&(mrc->dens[permuted_index(ordermode,i,mrc->xdim,mrc->ydim,mrc->zdim)]),sizeof(float),1,fpin);
